...
def _parse_search_url(url):
    ...
    # Strip fragment (#...) — Starlark has no URL parser
    url_clean = url
    if "#" in url_clean:
    ...
    # Pattern 1: /artifact/<group>/<artifact> path
    if "/artifact/" in url_clean:
        ...
        if len(path_parts) >= 2:
            group    = path_parts[0]
            artifact = path_parts[1].split("?")[0]  # strip any trailing query

    # Pattern 2: query string with g: and a: params
    if not group and ("g:" in url_clean or "g%3A" in url_clean):
        # Normalise URL-encoded colons
        q_part = url_clean.replace("g%3A", "g:").replace("a%3A", "a:")
...
def fetch_versions(ctx, group, artifact, maven_base_url = "https://search.maven.org"):
    """
